...
//    UITableViewCell *cell1 = [tableView dequeueReusableCellWithIdentifier:@"av"];
    
    
    
//    if (cell1 == nil) { NSLog(@"shit");}
//    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"bv"];
...
- (UIImage *)imageForExhibit:(NSString*)exhibitID
{
    for (id exh in _exhibits){
        if ([exh isKindOfClass:[Exhibit class]]) {
            Exhibit *e = exh;
            if ([e.ID isEqualToString:exhibitID]){
                return e.images[0];
            }
        }
    }
